...
// RUN: grep -Ev "//\s*[A-Z-]+:" %s > %t.cpp
// RUN: loop-convert -prefer-auto . %t.cpp -- -I %S/Inputs \
// RUN:           && FileCheck -check-prefix=CAUTO -input-file=%t.cpp %s
// RUN: rm -rf %t.cpp
...
void f() {
  ...
  // CAUTO: for (auto {{(const ) ?&? ?}}[[VAR:[a-z_]+]] : s)
  ...
  // CAUTO: for (auto {{(const ) ?&? ?}}[[VAR:[a-z_]+]] : *ps)
  // CHECK-NEXT: printf("s has value %d\n", [[VAR]].x);
  ...
  // CAUTO: for (auto {{&? ?}}[[VAR:[a-z_]+]] : intmap)
